...
const Sidebar: React.FC<SidebarProps> = ({ onToggle }) => {
  ...
  return (
    <div 
      className={`fixed top-0 left-0 h-screen bg-[#1e293b] transition-all duration-300 ease-in-out z-50 ${
        collapsed ? 'w-16' : 'w-56'
      }`}
    >
      <div className="flex flex-col h-full">
        ...
        <button
          ...
          className="absolute -right-2.5 top-20 bg-blue-600 text-white p-1 rounded-full shadow-lg hover:bg-blue-700 transition-colors"
        >
          <svg 
            ...
            {menuItems.map((item) => {
              ...
              return (
                <li key={item.name}>
                  <Link
                    href={item.path}
                    className={`flex items-center px-2.5 py-2 rounded-lg transition-all duration-200 group ${
                      isActive 
                        ? 'bg-blue-600 text-white' 
                        : 'text-gray-400 hover:bg-blue-600/10 hover:text-blue-500'
                    }`}
                  >
                    ...
                    {!collapsed && (
                      <span className={`ml-2 text-sm font-medium ${isActive ? 'text-white' : ''}`}>
                        {item.name}
                      </span>
                    )}
                    {isActive && !collapsed && (
