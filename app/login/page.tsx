...
const LoginPage = () => {
  ...
  const fullTitle = 'Vehicle Monitoring';
    ...
    const interval = setInterval(() => {
      setTypedTitle(prev => {
        if (prev.length < fullTitle.length) {
          return prev + fullTitle[prev.length];
        } else {
    ...
    <div className="login-container">
      <div className="login-card-flip">
        <div className={`login-card-inner${isFlipped ? ' flipped' : ''}`}> 
          ...
          <div className="login-card login-card-back">
            <div className="login-header flex flex-col items-center mb-6">
              <Image src="/logo1.png" alt="apml-logo" width={100} height={100} />
              <h1 className="login-title text-3xl font-bold text-white h-12 min-h-[48px]">
                {typedTitle}
