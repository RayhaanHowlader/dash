...
export default function MXLPage() {
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/vehicles?group=LINE_MXL');
  ...
  const handleExcelExport = () => {
    try {
      ...
      const statusGroups = {
        'Available': filteredVehicles.filter(v => v.currentTripStatus === 'available'),
        'In Transit': filteredVehicles.filter(v => v.currentTripStatus === 'in-transit'),
        'At Unloading': filteredVehicles.filter(v => v.currentTripStatus === 'at-unloading'),
        'Empty Movement': filteredVehicles.filter(v => v.currentTripStatus === 'empty-movement'),
        'Off Duty': filteredVehicles.filter(v => v.currentTripStatus === 'off-duty'),
        'At Pickup': filteredVehicles.filter(v => v.currentTripStatus === 'at-pickup'),
        'Enroute for Pickup': filteredVehicles.filter(v => v.currentTripStatus === 'enroute-for-pickup'),
        'Maintenance': filteredVehicles.filter(v => v.currentTripStatus === 'maintenance')
      };
      ...
      Object.entries(statusGroups).forEach(([status, vehicles]) => {
        ...
        const includesPKMS = status !== 'Available'; // PKMS only for non-available vehicles
      ...
      // Add to workbook
      XLSX.utils.book_append_sheet(wb, ws, "MXL Vehicles");
      
      // Write workbook and trigger download
      XLSX.writeFile(wb, `MXL_Vehicles_${new Date().toISOString().split('T')[0]}.xlsx`);
    } catch (err) {
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center">
                <Logo />
                <div className="ml-4">
                  <h1 className="text-2xl font-bold text-white">MXL VEHICLE</h1>
                  <p className="text-gray-400">Monitor and manage your MXL VEHICLE fleet operations</p>
                </div>
              </div>
