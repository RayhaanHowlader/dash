...
// import Card from '../../components/Card';f
...
// Add blink animation style for halt hours
// const blinkStyle = `
// @keyframes blink-bg {
//   0%, 100% { background-color: rgb(210, 6, 6); }
//   50% { background-color: rgb(154, 5, 12); }
// }
// .blink-bg {
...
export default function SXLPage() {
  ...
  const fetchVehicles = async (isRefresh = false) => {
      ...
      setLoadProgress(30); // Update progress after vehicles fetch
        ...
        if (vehicleData.length === 0) {
          ...
          // Extract all vehicle numbers for batch requests
          const vehicleNumbers = vehicleData.map((v: Vehicle) => v.vehicleNumber);
          
          setLoadProgress(40); // Update progress
          ...
          // Create chunks of vehicle numbers (max 50 per request to avoid URL length issues)
          ...
          setLoadProgress(70); // Update progress after batch requests
        ...
        setLoadProgress(80); // Update progress after parsing JSON
        ...
        setLoadProgress(90); // Update progress
  ...
  // Helper function to sort vehicles by halting hours (not a React hook)
          ...
          <table className="dashboard-table" style={{ width: '100%' }}>
            ...
            <tbody>
              {sortedVehicles.map((vehicle) => {
                ...
                // Get status indicators for each column
  ...
  const handleDownloadChart = () => {
    ...
    if (chartRef.current) {
      ...
      lineChartLink.download = `sxl_branch_chart_${new Date().toISOString().slice(0, 10)}.png`;
      ...
      import('html2canvas').then((htmlCanvas) => {
        ...
        if (barChartRef.current && barChartRef.current.containerRef.current) {
          htmlCanvas.default(barChartRef.current.containerRef.current, {
            ...
            barChartLink.download = `sxl_bar_chart_with_tables_${new Date().toISOString().slice(0, 10)}.png`;
            ...
            if (ctx) {
              ...
              // Convert to image and download
              const barChartLink = document.createElement('a');
              barChartLink.download = `sxl_bar_chart_${new Date().toISOString().slice(0, 10)}.png`;
  ...
  return (
    <div className={`min-h-screen w-full bg-[var(--bg-primary)] ${ibmPlexSans.variable} ${montserrat.variable}`} style={{
      ...
      <div className="dashboard-container" style={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        {/* Branch Availability Line Chart is now hidden for download only */}
        ...
        <header className="header-container" style={{
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center">
              <Logo />
              <div className="ml-4">
                <h1 className="text-3xl font-bold text-white tracking-wide">SXL VEHICLE</h1>
                <p className="text-gray-400 text-sm">Monitor and manage your SXL VEHICLE fleet operations</p>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
              ...
              <Button
                variant="primary"
                onClick={() => handleExport(vehicles, 'SXL Vehicles')}
        ...
        <div className="relative w-full">
          {/* Scroll buttons with enhanced glass effect */}
          <button 
        ...
        <div style={{ position: 'absolute', left: '-9999px', top: 0, width: '1872px', height: 'auto', pointerEvents: 'none', zIndex: -1 }} aria-hidden="true">
          <BarChart ref={barChartRef} data={barChartData} logoUrl="/logo.png" pageName="SXL" />
        </div>
    </div>
  );
} 
