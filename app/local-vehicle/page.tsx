...
export default function LocalVehiclePage() {
  ...
  const handleDownloadChart = () => {
    ...
    if (chartRef.current) {
      ...
      lineChartLink.download = `local_vehicle_branch_chart_${new Date().toISOString().slice(0, 10)}.png`;
      ...
      import('html2canvas').then((htmlCanvas) => {
        ...
        if (barChartRef.current && barChartRef.current.containerRef.current) {
          htmlCanvas.default(barChartRef.current.containerRef.current, {
            ...
            barChartLink.download = `local_vehicle_bar_chart_with_tables_${new Date().toISOString().slice(0, 10)}.png`;
            ...
            if (ctx) {
              ...
              // Convert to image and download
              const barChartLink = document.createElement('a');
              barChartLink.download = `local_vehicle_bar_chart_${new Date().toISOString().slice(0, 10)}.png`;
    ...
    <div className={`min-h-screen w-full bg-black ${ibmPlexSans.variable} ${montserrat.variable}`} style={{
      ...
      <div className="dashboard-container" style={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', left: '-9999px', top: 0, width: '1872px', height: 'auto', pointerEvents: 'none', zIndex: -1 }} aria-hidden="true">
          <BarChart ref={barChartRef} data={barChartData} logoUrl="/logo.png" pageName="LOCAL VEHICLE" />
        </div>
